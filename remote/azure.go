...
type AzureRemote struct {
	...
	// Need to look up Azure equivalent
	Container  *blob.Container
	client     *blob.BlobStorageClient
}

var (
	AzureDefaultRegion = "us-east-1"
)

func NewAzureRemote(config RemoteConfig) (*AzureRemote, error) {
	azure, err := newAzureClient(config)
	if err != nil {
		return &AzureRemote{}, err
	}
...
func newAzureClient(config RemoteConfig) (*blob.BlobStorageClient, error) {
	auth, err := getAzureAuth(config)
	if err != nil {
		return &blob.BlobStorageClient{}, err
	}
	...
	if len(regQuery) > 0 && regQuery[0] != "" {
		...
		regionName = AzureDefaultRegion
	}

	region := azure.Regions[regionName]

	return azure.New(auth, region), nil
}

// determine the s3 auth from various sources
func getAzureAuth(config RemoteConfig) (azure.Auth, error) {
	azureconfig := config.Config.Azure
